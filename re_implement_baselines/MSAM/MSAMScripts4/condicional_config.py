...
if MODEL_TYPE not in ["CE","LE","CE_MSAM","LE_MSAM","CE_MSAM_CLQ","LE_MSAM_CLQ"]:
    ...
    exit(1)
if DATA_TYPE not in ["top50", "mimic4_icd9"]:
    print("ERROR: Invalid DATA_TYPE = {}. You must select a valid data type from the following list {}".format(DATA_TYPE,["top50", "mimic4_icd9"]))
    exit(1)
...
elif ("CE" in MODEL_TYPE) & (DATA_TYPE == "mimic4_icd9"):
