...
def main():
    ...
    elif DATA_TYPE == "mimic4_icd9":
        print("Load text...")
        with open(DATA_PATH + "/mimic4_icd9_train.pkl", "rb") as file:
            train_data = pickle.load(file)
            train_texts = train_data["text"].tolist()
        with open(DATA_PATH + "/mimic4_icd9_val.pkl", "rb") as file:
            val_data = pickle.load(file)
            val_texts = val_data["text"].tolist()
        with open(DATA_PATH + "/mimic4_icd9_test.pkl", "rb") as file:
            test_data = pickle.load(file)
            test_texts = test_data["text"].tolist()
            
        
        print("Load 1hot labels...")
        train_1hot = np.load(DATA_PATH + '/mimic4_icd9_train_1hot.npz')['arr_0']
        val_1hot = np.load(DATA_PATH + '/mimic4_icd9_val_1hot.npz')['arr_0']
        test_1hot = np.load(DATA_PATH + '/mimic4_icd9_test_1hot.npz')['arr_0']
