...
class MSAM(PreTrainedModel):
    ...
    def __init__(self,config):
        ...
        if not os.path.exists(OUTPUT_DIR + "/def_embeddings.pkl"):
                    
            with open("../sample_data/{}/icd_mimic4_icd10_def.pkl".format(DATA_TYPE), 'rb') as f:
            ...
            self.out_code = pad_sequence(self.out_code,batch_first = True,padding_value=0).to(DEVICE)
            self.synomns_vector = self.out_code
    ...
    def forward(self,
        ...
        sentences_encodings = encoder_outputs[0]
        q =self.out_code.unsqueeze(1) # Synonyms
        # print(q.shape)
        # print(self.synomns_vector.shape)
