...
def main():
    ...
    if DATA_TYPE == "top50":
        ...
        print("Load text...")
        with open(DATA_PATH + "/mimic3_train.pkl", "rb") as file:
            train_data = pickle.load(file)
            train_texts = train_data["TEXT"].tolist()
        with open(DATA_PATH + "/mimic3_val.pkl", "rb") as file:
            val_data = pickle.load(file)
            val_texts = val_data["TEXT"].tolist()
        with open(DATA_PATH + "/mimic3_test.pkl", "rb") as file:
            test_data = pickle.load(file)
            test_texts = test_data["TEXT"].tolist()
            
        
        print("Load 1hot labels...")
        train_1hot = np.load(DATA_PATH + '/mimic3_train_1hot.npz')['arr_0']
        val_1hot = np.load(DATA_PATH + '/mimic3_val_1hot.npz')['arr_0']
        test_1hot = np.load(DATA_PATH + '/mimic3_test_1hot.npz')['arr_0']
